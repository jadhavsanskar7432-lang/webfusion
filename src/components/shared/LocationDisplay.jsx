...
export function LocationDisplay({ location, isRevealed = false, className }) {
  ...
  return (
    <div className={cn('rounded-[4px] border border-ink/10 bg-card p-4', className)}>
      <div className="flex items-start gap-3">
        <MapPin size={18} className="text-moss shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          ...
          {isRevealed && (
            <a
              ...
              className="inline-flex items-center gap-1.5 mt-2 text-xs font-medium text-moss hover:text-moss/80 transition-colors duration-150"
            >
              <Navigation size={12} />
              Get Directions
            </a>
